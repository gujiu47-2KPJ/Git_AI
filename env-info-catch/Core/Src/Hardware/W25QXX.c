...
/* 外部 SPI 句柄声明 */
extern SPI_HandleTypeDef hspi1;//声明这个句柄是在其他函数里面被定义的，编译的时候会先略过它先别报错。
...
uint8_t W25QXX_ReadSR(void)
{
    ...
    W25QXX_CS_LOW();
    HAL_SPI_Transmit(&hspi1, &cmd, 1, 100);//这里使用了外部spi句柄，所以前面要提前声明
...
{
    uint16_t page_remain = W25QXX_PAGE_SIZE - (addr % W25QXX_PAGE_SIZE);//确定还一页中也就是一列，还有多少位置可以写
...
uint8_t W25QXX_SectorErase(uint32_t addr)
{
    ...
    /* 地址对齐到 4KB 边界 */
    addr &= ~(W25QXX_SECTOR_SIZE - 1);//利用位运算将地址末尾的“零头”（偏移量）强制清零，从而定位到整块扇区的起始位置
